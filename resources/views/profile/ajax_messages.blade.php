	<div class="chatting_section">
			...
			<div class="chats @if($user_msg[$i]['from_user_id'] == Auth::user()->id)
				...
				<div class="user_details media">
					...
					<div class="user_profile bg-red media-left">
						...
						<?php if (isset($userobj->profile_image) && $userobj->profile_image != null && file_exists(public_path() . '/user_img/' . $userobj->profile_image)) { ?>

							<img src="{{ asset("/public/user_img/".$userobj->profile_image) }}" alt="{{ucfirst($userobj->first_name)}} {{ ucfirst($userobj->last_name) }}">

						<?php } else {

							echo ucfirst(substr($userobj->first_name, 0, 1));
		...
		<div class="chat_typing">
			...
			<div class="input-group">
				...
				<input type="text" id="message" class="form-control" placeholder="Hi, Type Here ..." name="search" <?php if (isset($connection->is_block) && $connection->is_block==1){ ?> disabled="disabled" <?php } ?>>

				<div class="input-group-btn">
					<?php $connection = getConnection(Auth::user()->id,$touserobj->id); ?>
					<button class="btn btn-default send" id="send" type="submit" <?php if (isset($connection->is_block) && $connection->is_block==1){ ?> disabled="disabled" <?php } ?>>Send</button>
