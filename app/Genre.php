...
FROM `genres` inner join `blogs` on `genres`.`id` = `blogs`.`blog_genre` inner join `users` on `blogs`.`created_by` = `users`.`id`  where `blogs`.`is_deleted` = '0'  and `blogs`.`blog_status` = 1 and `blogs`.`created_by`= $user_id and `users`.`id`=$user_id and `users`.`is_verified` = 1 and `users`.`is_delete` = '0' group BY new_parent) and `genres`.`is_deleted` = 'N' and `genres`.`is_published` = 'Y' and `parent_genre_id` = 0 and `user_preferences`.`is_delete` = 0 ");
    		return $user;
    	}else{
    		$user = DB::select("SELECT DISTINCT `genres`.*,`user_preferences`.`preference_id` FROM `genres`
inner join `user_preferences` on `user_preferences`.`preference_id` = `genres`.`id`
WHERE `genres`.id in (SELECT CASE `genres`.parent_genre_id WHEN 0 THEN `genres`.id ELSE `genres`.`parent_genre_id` END as new_parent
FROM `genres` inner join `blogs` on `genres`.`id` = `blogs`.`blog_genre` inner join `users` on `blogs`.`created_by` = `users`.`id`  where `blogs`.`is_deleted` = '0'  and `blogs`.`blog_status` = 1 and `users`.`is_verified` = 1 and `users`.`is_delete` = '0' group BY new_parent) and `genres`.`is_deleted` = 'N' and `genres`.`is_published` = 'Y' and `parent_genre_id` = 0 and `user_preferences`.`is_delete` = 0 ");
    	return $user;
    }
...
WHERE `genres`.`parent_genre_id`= $id and `blogs`.`is_deleted` = '0'  and `blogs`.`blog_status` = 1 and `blogs`.`created_by`= $user_id and `users`.`id`=$user_id and `users`.`is_verified` = 1 and `users`.`is_delete` = '0' and `genres`.`is_deleted` = 'N' and `genres`.`is_published` = 'Y' and `parent_genre_id` = 0 and `user_preferences`.`is_delete` = 0 ");
    		return $user;
    	}else{
    		$user = DB::select("SELECT DISTINCT `genres`.*,`user_preferences`.`preference_id` FROM `genres`
inner join `user_preferences` on `user_preferences`.`preference_id` = `genres`.`id`
inner join `blogs` on `genres`.`id` = `blogs`.`blog_genre` 
inner join `users` on `blogs`.`created_by` = `users`.`id`
WHERE `genres`.`parent_genre_id`= $id  and `blogs`.`is_deleted` = '0'  and `blogs`.`blog_status` = 1 and `users`.`is_verified` = 1 and `users`.`is_delete` = '0' and `genres`.`is_deleted` = 'N' and `genres`.`is_published` = 'Y' and `parent_genre_id` = 0 and `user_preferences`.`is_delete` = 0 ");
    		return $user;
    }
}
